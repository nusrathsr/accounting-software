...
export default function ViewPurchase() {
  ...
  const filteredPurchases = purchases.filter((purchase) => {
    ...
    // Search filter - now includes variant
    const matchesSearch =
      ...
      (purchase.variant && purchase.variant.toLowerCase().includes(s));
  ...
  const generateSinglePurchaseHtml = (purchase) => {
  ...
  return `
    <html>
      <head>
        <title>Purchase Bill - ${purchase.purchaseOrderNumber}</title>
        <style>
      ...
      <body>
        ...
        <div class="section">
          <strong>Purchase Order No:</strong> ${purchase.purchaseOrderNumber}<br>
          <strong>Supplier:</strong> ${purchase.supplierName || "N/A"}<br>
          <strong>Date:</strong> ${purchase.purchaseDate ? new Date(purchase.purchaseDate).toLocaleDateString('en-IN') : "-"}
        </div>
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td>${purchase.product || "N/A"}</td>
              <td>${purchase.variant || "-"}</td>
              <td>${purchase.quantity || 0}</td>
              <td class="amount">₹${parseFloat(purchase.unitPrice || 0).toFixed(2)}</td>
              <td class="amount">${purchase.tax || 0}%</td>
              <td class="amount">₹${parseFloat(purchase.totalAmount || 0).toFixed(2)}</td>
            </tr>
          </tbody>
        </table>

        <div class="section">
          <p><strong>Paid Amount:</strong> ₹${parseFloat(purchase.paidAmount || 0).toFixed(2)}</p>
          <p><strong>Status:</strong> <span class="${isPaid ? 'paid' : 'pending'}">${isPaid ? 'Paid' : 'Pending'}</span></p>
        </div>
      </body>
    </html>
  `;
                        ...
                        <tr key={idx} className="hover:bg-gray-50 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <PrintButton
                                printData={purchase}
                                generateHtml={() => generateSinglePurchaseHtml(purchase)}
