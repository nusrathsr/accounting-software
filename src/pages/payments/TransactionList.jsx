...
//                       <td className="px-6 py-4">
//                         <div className={`text-sm font-semibold ${transaction.type === 'payment' ? 'text-red-600' : 'text-green-600'
//                           }`}>
...
//                           <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${transaction.paymentStatus
//                               ? 'bg-green-100 text-green-700'
//                               : 'bg-red-100 text-red-700'
//                             }`}>
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const [paymentsResponse, purchasesResponse] = await Promise.all([
        fetch(`${baseURL}/payments`).catch(() => ({ ok: false })),
        fetch(`${baseURL}/purchases`).catch(() => ({ ok: false }))
      ]);
