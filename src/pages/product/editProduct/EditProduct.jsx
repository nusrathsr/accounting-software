...
const EditProduct = () => {
  ...
  return (
    <div  className="p-4 w-full  sm:px-8 lg:px-12">
      <div className=" p-6 shadow rounded max-w-5xl mx-auto ">
        <h2 className="text-2xl font-bold mb-6 text-gray-700">Edit Product (Step {step}/3)</h2>
          ...
          {step === 1 && (
            <>
              <div>
                ...
                <input type="text" name="name" value={product.name} onChange={handleChange} className="w-full p-2 border rounded bg-transparent" />
              </div>

              <div>
                <label className="block font-medium">Category</label>
                <select name="category" value={product.category} onChange={handleChange} className="w-full p-2 border rounded bg-transparent">
              ...
              {subcategories.length > 0 && (
                <div>
                  ...
                  <select name="subcategory" value={product.subcategory} onChange={handleChange} className="w-full p-2 border rounded bg-transparent">
                    <option value="">Select</option>
              ...
              <div>
                ...
                <input type="text" name="brand" value={product.brand} onChange={handleChange} className="w-full p-2 border rounded bg-transparent" />
              </div>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div>
                ...
                {product.sizes.map((item, index) => (
                  <div key={index} className="flex gap-4 mb-2">
                    <input type="text" placeholder="Size" value={item.size} onChange={(e) => handleSizeChange(index, 'size', e.target.value)} className="p-2 border rounded bg-transparent w-1/3" />
                    <input type="number" placeholder="Quantity" value={item.quantity} onChange={(e) => handleSizeChange(index, 'quantity', e.target.value)} className="p-2 border rounded bg-transparent w-1/3" />
              ...
              <div>
                ...
                <input type="number" name="sellingPrice" value={product.sellingPrice} onChange={handleChange} className="w-full p-2 border rounded bg-transparent" />
              </div>
