...
//                               <p key={idx} className="font-semibold text-green-600">₹{size.sellingPrice} <span className="text-gray-500 text-xs">Cost: ₹{size.purchasePrice}</span> ({size.size})</p>
//                             ))
//                           ) : (
//                             <p className="font-semibold text-green-600">₹{prod.sellingPrice} <span className="text-gray-500 text-xs">Cost: ₹{prod.purchasePrice}</span></p>
//                           )}
//                         </div>
//                       </td>
//                       <td className="px-6 py-4 whitespace-nowrap">
...
//                           <div className="text-xs text-gray-500 mt-1">
//                             {prod.sizes.map((size, idx) => (
//                               <span key={idx} className="mr-2">{size.size}({size.quantity})</span>
//                             ))}
//                           </div>
//                         )}
//                       </td>
                  ...
                  {paginatedProducts.map((prod) => (
                    <tr key={prod._id} className="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <div className="text-sm">
                          {prod.variants && prod.variants.length > 0 ? (
                            prod.variants.map((v, idx) => (
                              <p key={idx} className="font-semibold text-green-600">
                                ₹{v.sellingPrice || 0}
                                <span className="text-gray-500 text-xs"> Cost: ₹{v.purchasePrice || 0}</span>
                                <span className="text-gray-400 text-xs ml-1">({v.sizeOrWeight || "N/A"})</span>
                              </p>
                            ))
                          ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {/* Show breakdown if multiple variants */}
                        {prod.variants && prod.variants.length > 0 && (
                          <div className="text-xs text-gray-500 mt-1">
                            {prod.variants.map((v, idx) => (
                              <span key={idx} className="mr-2">
                                {v.sizeOrWeight || "N/A"} ({v.quantity || 0})
                              </span>
                            ))}
                          </div>
                        )}
