...
const AddCustomer = () => {
  ...
  // Fetch customer categories for dropdown
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await api.get("/customer-categories"); // GET all categories
        setDynamicCategories(res.data); // set the fetched categories
      } catch (error) {
  ...
  const customerTypes = [...staticTypes, ...dynamicCategories.map(cat => ({ value: cat.name, label: cat.name }))];
