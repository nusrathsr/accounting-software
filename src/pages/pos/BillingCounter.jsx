...
const BillingCounter = () => {
  ...
  // POS quick form (for scanning / variant search)
  const [formData, setFormData] = useState({
    variantId: "",
    variantName: "",
    quantity: 1,
    unitPrice: 0,
    discount: 0, // in % while adding; we keep discountAmount in cart
    tax: 0,
    customer: "",
    number: ""
  });
  ...
  // Payment state (supports split)
  const [paymentInfo, setPaymentInfo] = useState({
    paymentMode: "single",
    singlePaymentMethod: "cash",
    paymentStatus: false,
    splitPayments: [{ method: "cash", amount: 0, paid: false }],
  });
  ...
  useEffect(() => {
    // regenerate invoice when component mounts or when needed
  ...
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    // keep numeric fields as numbers where appropriate
    setFormData(prev => ({ ...prev, [name]: value }));
    if (name === "variantName") {
      // search variants from GlobalContext product
      if (value.length === 0) {
        setSuggestions([]);
        return;
      }
      ...
      for (let p of product || []) {
        for (let v of p.variants || []) {
          if (v.variantName?.toLowerCase().includes(value.toLowerCase()) || (v.barcode && v.barcode.includes(value))) {
            matches.push({
              variantId: v.variantId || v._id,
              variantName: v.variantName,
              unitPrice: Number(v.sellingPrice) || 0,
              taxPercentage: Number(v.taxPercentage) || 0,
  ...
  // --- Cart operations ---
  const handleAddToCart = () => {
    if (!formData.variantId || !formData.variantName) {
      Swal.fire({ icon: 'warning', title: 'Select Variant', text: 'Please select a variant.' });
      return;
    }

    // find actual variant in global products to check stock if available
    let selectedVariant = null;
    for (let p of product || []) {
      selectedVariant = (p.variants || []).find(v => v.variantId === formData.variantId || v._id === formData.variantId);
      if (selectedVariant) break;
    }

    if (selectedVariant && selectedVariant.quantity === 0) {
      Swal.fire({ icon: 'warning', title: 'Out of stock', text: 'This item is out of stock.' });
      return;
    }

    if (selectedVariant && Number(formData.quantity) > selectedVariant.quantity) {
      Swal.fire({ icon: 'warning', title: 'Insufficient Stock', text: `Only ${selectedVariant.quantity} available.` });
      return;
    }

    // compute discount amount (we accept discount % from the small form)
    const qty = Number(formData.quantity) || 1;
    const unit = Number(formData.unitPrice) || 0;
    const discountPercent = Number(formData.discount) || 0;
    const discountAmount = (unit * qty * discountPercent) / 100;
    const taxPercent = Number(formData.tax) || 0;
    const taxAmount = taxPercent ? ((unit * qty - discountAmount) * taxPercent) / 100 : 0;
    const lineTotal = (unit * qty - discountAmount) + taxAmount;

     const newItem = {
    slNo: cart.length + 1,
    variantId: selectedVariant._id, // <--- Use actual MongoDB ObjectId
    variantName: selectedVariant.name || formData.variantName,
    productId: selectedVariant.productId || selectedVariant._id, // productId ObjectId
    quantity: qty,
    unitPrice: unit,
    discountPercent,
    discount: discountAmount,
    ...
    // reset scanning fields but keep selected customer
    setFormData(prev => ({
      ...prev,
      variantId: "",
      variantName: "",
  ...
  const removeItem = (slNo) => {
    setCart(prev => prev.filter(i => i.slNo !== slNo).map((it, idx) => ({ ...it, slNo: idx + 1 })));
  };
  ...
  const totalTax = cart.reduce((acc, item) => acc + (item.tax || 0), 0);
  const grandTotal = subTotal + totalTax;

  // --- Payment handlers (split supported) ---
  const handlePaymentChange = (e) => {
    const { name, value } = e.target;
  ...
  const handleSplitPaymentChange = (index, field, value) => {
    const updated = [...paymentInfo.splitPayments];
    updated[index][field] = field === "paid" ? value : Number(value);
    setPaymentInfo(prev => ({ ...prev, splitPayments: updated }));
  };

  const addSplitPayment = () => {
    setPaymentInfo(prev => ({ ...prev, splitPayments: [...prev.splitPayments, { method: "cash", amount: 0, paid: false }] }));
  };
  ...
  const getTotalSplitAmount = () => paymentInfo.splitPayments.reduce((acc, p) => acc + (Number(p.amount) || 0), 0);
  const getRemainingAmount = () => grandTotal - getTotalSplitAmount();
  const isSplitPaymentComplete = () => Math.abs(getTotalSplitAmount() - grandTotal) < 0.01;

  // --- Build invoice object following AddSale schema ---
  const createInvoiceData = () => {
    const formattedProducts = cart.map(item => ({
      productId: item.productId || null,
      variantId: item.variantId || null,
      name: item.variantName,
      sizeOrWeight: "", // billing counter doesn't track size in this compact UI; can be added
      quantity: Number(item.quantity),
      unitPrice: Number(item.unitPrice),
      discount: Number(item.discount) || 0, // absolute discount amount
      tax: Number(item.tax) || 0, // absolute tax amount
      total: Number(item.lineTotal) || 0,
    }));

    const subtotal = formattedProducts.reduce((s, p) => s + (p.unitPrice * p.quantity) - (p.discount || 0), 0);
    const totalTax = formattedProducts.reduce((s, p) => s + (p.tax || 0), 0);
    ...
    const base = {
      ...
      paymentStatus: paymentInfo.paymentStatus === true || paymentInfo.paymentStatus === "true" ? true : false,
    };

    if (paymentInfo.paymentMode === "single") {
      base.singlePaymentMode = paymentInfo.singlePaymentMethod || "cash";
    } else {
      base.splitPayments = paymentInfo.splitPayments.map(p => ({ method: p.method, amount: Number(p.amount) || 0, paid: !!p.paid }));
    }

    return base;
  };

  // --- Generate printable HTML (simple but clear) ---
  const generateInvoiceHTML = (invoiceData) => {
    const productsRows = (invoiceData.products || []).map((p, i) => `
      <tr>
        ...
        <td style="padding:8px;border:1px solid #ddd;text-align:right">₹${(p.discount||0).toFixed(2)}</td>
        <td style="padding:8px;border:1px solid #ddd;text-align:right">₹${(p.tax||0).toFixed(2)}</td>
        <td style="padding:8px;border:1px solid #ddd;text-align:right">₹${(p.total||0).toFixed(2)}</td>
      </tr>
    `).join("");

    const paymentsHtml = invoiceData.paymentMode === 'single'
      ? `<div style="margin-top:8px;">Payment: ${invoiceData.singlePaymentMode || 'cash'} — ₹${invoiceData.totalAmount.toFixed(2)}</div>`
      : ((invoiceData.splitPayments || []).map(sp => `<div>${sp.method} — ₹${Number(sp.amount).toFixed(2)} ${sp.paid ? '(Paid)' : '(Pending)'}</div>`).join(""));
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <div>Date: ${new Date(invoiceData.date).toLocaleString()}</div>
            <div>Customer: ${invoiceData.customerName || 'Walk-in'}</div>
            ${invoiceData.number ? `<div>Phone: ${invoiceData.number}</div>` : ''}
          </div>
          ...
          <div style="margin-top:14px; text-align:right;">
            ...
            <div>Total Discount: -₹${(invoiceData.products.reduce((s,p)=>s+(p.discount||0),0)).toFixed(2)}</div>
            <div>Total Tax: +₹${invoiceData.tax.toFixed(2)}</div>
            <h3 style="margin-top:6px">Grand Total: ₹${invoiceData.totalAmount.toFixed(2)}</h3>
            ${paymentsHtml}
            <div style="margin-top:12px;">Status: ${invoiceData.paymentStatus ? 'PAID' : 'PAYMENT PENDING'}</div>
          </div>

          <div style="margin-top:30px; text-align:center;">Thank you for shopping with us!</div>
        </body>
      </html>
    `;
  };
  ...
  const openAndPrint = (html) => {
    ...
    newWindow.document.close();
    // give a tiny delay before print to ensure DOM ready
    setTimeout(() => {
      newWindow.focus();
      newWindow.print();
      // optionally newWindow.close();
    }, 250);
  };

  // --- Place sale (save to backend) ---
  const handlePlaceSale = async () => {
    if (cart.length === 0) {
      Swal.fire({ icon: 'warning', title: 'Cart empty', text: 'Add at least one item.' });
      return;
    }

    // minimal validation for split payments
    if (paymentInfo.paymentMode === 'split') {
      const splitTotal = getTotalSplitAmount();
      if (splitTotal > 0 && splitTotal > grandTotal + 0.01) {
        Swal.fire({ icon: 'warning', title: 'Split total exceeds invoice', text: `Split payments exceed total by ₹${(splitTotal - grandTotal).toFixed(2)}` });
        return;
      }
    }
    ...
    try {
      ...
      console.log("➡️ InvoiceData to POST:", invoiceData); // <-- Add this

      // POST to /api/sales
      const res = await api.post("/sales", invoiceData);

      Swal.fire({ icon: 'success', title: 'Sale saved', text: 'Invoice saved successfully.' });

      // Auto-print the saved invoice (we will use our generated HTML)
      // If backend returns the saved invoice with id and final data, you can use res.data.sale instead.
      const invoiceForPrint = res?.data?.sale || invoiceData;
      const html = generateInvoiceHTML(invoiceForPrint);
      openAndPrint(html);
      ...
      Swal.fire({ icon: 'error', title: 'Save failed', text: 'Could not save invoice. Check server.' });
    }
  };

  // --- UI (keeps your original layout, adapted to integrated logic) ---
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-8 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 px-8 py-6">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate(-1)} className="p-2 text-white hover:bg-white/10 rounded-lg transition-all duration-200">
                <FaArrowLeft className="w-5 h-5" />
              </button>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3">
                <FaShoppingCart className="w-6 h-6 text-white" />
            ...
            <div className="space-y-6">
              <div className="relative">
                ...
                {suggestions.length > 0 && (
                  <ul className="absolute mt-1 border rounded-xl bg-white shadow-lg w-full max-h-40 overflow-y-auto z-50">
                    {suggestions.map((v, i) => (
                      <li key={i} className="p-3 cursor-pointer hover:bg-blue-50 transition-colors duration-150 border-b last:border-b-0"
                          onClick={() => selectVariant(v)}>
                        <div className="flex justify-between items-center">
                          <span className="font-medium text-gray-800">{v.variantName}</span>
                          <span className="text-blue-600 font-semibold">₹{v.unitPrice}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <FaRupeeSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                    <input type="number" value={formData.unitPrice} readOnly className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-100 text-gray-600" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <FaPercent className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                    <input type="number" value={formData.tax || 0} readOnly className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-100 text-gray-600" />
                  </div>
                </div>
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <FaUser className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                  <input type="text" name="customer" value={formData.customer} onChange={(e) => {
                    const value = e.target.value;
                    setFormData(prev => ({ ...prev, customer: value }));
                    const filtered = (customers || []).filter(c =>
                      (c.type === "Wholesale Customer" || c.type === "Retail Customer") &&
                      (c.name?.toLowerCase().includes(value.toLowerCase()) || c.phone?.includes(value))
                    );
                    setCustomerSuggestions(filtered);
                  }}
                    placeholder="Type customer name or phone (optional)" className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50" />
                </div>
                {customerSuggestions.length > 0 && (
                  <ul className="absolute mt-1 border rounded-xl bg-white shadow-lg w-full max-h-40 overflow-y-auto z-50">
                    {customerSuggestions.map(c => (
                      <li key={c._id} className="p-3 cursor-pointer hover:bg-blue-50" onClick={() => {
                        setFormData(prev => ({ ...prev, customer: c.name, number: c.phone || "" }));
                        setCustomerSuggestions([]);
                      }}>
              ...
              <div className="flex justify-end">
                <button type="button" onClick={handleAddToCart} className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl flex items-center gap-2">
                  <FaPlus /> Add to Cart
                </button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-8 overflow-hidden">
          <div className="p-8">
            <h2 className="text-xl font-semibold text-gray-900 mb-6 flex items-center gap-2"><FaShoppingCart /> Shopping Cart ({cart.length})</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                ...
                <tbody>
                  {cart.length === 0 ? (
                    <tr><td colSpan="8" className="px-4 py-8 text-center text-gray-500">Your cart is empty.</td></tr>
                  ) : cart.map((item, idx) => (
                    <tr key={item.slNo}>
                      <td className="px-4 py-4">{item.slNo}</td>
                      <td className="px-4 py-4">{item.variantName}</td>
                      <td className="px-4 py-4 text-center">{item.quantity}</td>
                      <td className="px-4 py-4 text-right">₹{item.unitPrice.toFixed(2)}</td>
                      <td className="px-4 py-4 text-right">₹{(item.discount||0).toFixed(2)}</td>
                      <td className="px-4 py-4 text-right">₹{(item.tax||0).toFixed(2)}</td>
                      <td className="px-4 py-4 text-right font-semibold">₹{(item.lineTotal||0).toFixed(2)}</td>
                      <td className="px-4 py-4 text-center">
                        <button onClick={() => removeItem(item.slNo)} className="text-red-500 p-2"><FaTrash /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {cart.length > 0 && (
              <div className="mt-6 border-t pt-6 flex justify-end">
                <div className="w-80 space-y-3">
                  ...
                  <div className="flex justify-between text-red-600"><span>Total Discount:</span><span>-₹{totalDiscount.toFixed(2)}</span></div>
                  <div className="flex justify-between text-green-600"><span>Total Tax:</span><span>+₹{totalTax.toFixed(2)}</span></div>
                  <div className="border-t pt-3 flex justify-between text-xl font-bold text-blue-700"><span>Grand Total:</span><span>₹{grandTotal.toFixed(2)}</span></div>
                </div>
              </div>
            )}
          </div>
        ...
        {cart.length > 0 && (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-8 overflow-hidden">
            <div className="p-8">
              <h2 className="text-xl font-semibold text-gray-900 mb-6 flex items-center gap-2"><FaCreditCard /> Payment Information</h2>

              <div className="mb-6">
                <div className="flex gap-6">
                  <label className="flex items-center gap-3">
                    <input type="radio" name="paymentMode" value="single" checked={paymentInfo.paymentMode === "single"} onChange={handlePaymentChange} />
                    <span className="ml-2">Single Payment</span>
                  </label>
                  <label className="flex items-center gap-3">
                    <input type="radio" name="paymentMode" value="split" checked={paymentInfo.paymentMode === "split"} onChange={handlePaymentChange} />
              ...
              {paymentInfo.paymentMode === "single" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select name="singlePaymentMethod" value={paymentInfo.singlePaymentMethod} onChange={handlePaymentChange} className="w-full px-3 py-2 border rounded">
                      <option value="cash">Cash</option>
                      <option value="card">Card</option>
                      <option value="upi">UPI</option>
                      <option value="wallet">Wallet</option>
                  ...
                  <div className="flex items-center">
                    <label className="flex items-center gap-3">
                      <input type="checkbox" name="paymentStatus" checked={paymentInfo.paymentStatus} onChange={handleCheckboxChange} />
                ...
                <div className="space-y-6">
                  {paymentInfo.splitPayments.map((p, i) => (
                    <div key={i} className="p-4 border rounded flex gap-4 items-center">
                      <select value={p.method} onChange={(e) => handleSplitPaymentChange(i, 'method', e.target.value)} className="px-3 py-2 border rounded">
                        <option value="cash">Cash</option>
                        <option value="card">Card</option>
                        <option value="upi">UPI</option>
                        <option value="wallet">Wallet</option>
                      ...
                      <div className="flex-1">
                        <input type="number" min="0" step="0.01" value={p.amount} onChange={(e) => handleSplitPaymentChange(i, 'amount', e.target.value)} className="w-full px-3 py-2 border rounded" placeholder="Amount" />
                      </div>
                      <label className="flex items-center gap-2">
                        <input type="checkbox" checked={p.paid} onChange={(e) => handleSplitPaymentChange(i, 'paid', e.target.checked)} />
                        <span>Paid</span>
                      </label>
                      <button onClick={() => removeSplitPayment(i)} disabled={paymentInfo.splitPayments.length <= 1} className="text-red-500 px-3 py-2">Remove</button>
                    </div>
                  ))}
                  <div className="flex justify-between items-center">
                    <div>Total Split: ₹{getTotalSplitAmount().toFixed(2)}</div>
                    <div>Remaining: ₹{getRemainingAmount().toFixed(2)}</div>
                    <button onClick={addSplitPayment} className="px-4 py-2 bg-blue-50 rounded">Add Payment</button>
                  </div>
                </div>
              )}

              {/* Action buttons */}
              <div className="border-t pt-6 mt-8 flex justify-end gap-4">
                <button onClick={() => {
                  setCart([]); setFormData({ variantId: "", variantName: "", quantity: 1, unitPrice: 0, discount: 0, tax: 0, customer: "", number: ""});
                }} className="px-6 py-3 bg-gray-100 rounded">Clear Cart</button>

                <button onClick={handlePlaceSale} className="px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl flex items-center gap-2">
                  <FaPrint /> Place Sale & Print
                </button>
              </div>
            </div>
          </div>
